<template>
  <div class="bar-right" v-title="'买家订单'">
    <!-- <div class="oil-title-2">买家订单</div> -->
    ...
    <div class="main-details">
      <ul>
        <li v-for="(order, i) in orderOptions" :key="i">
          <order-item :is-system="isSystem" :order-data="order" :business-type="3" :checkOrder="jump2center" @order-delete="orderDelete"/>
        </li>
        <li v-if="isNullTable">
...
export default {
  ...
  data() {
    return {
      uploadUrl: url.buyImportRIgnitionData,
  ...
  methods: {
    // 交易方
    getNormalData() {
      this.getDataList(url.buyerOrderUp, this.form);
    },
    // 搜索传值
    orderSearchFn(sdata) {
    ...
    exportTemplate() {
      let obj = {
        ...
        buyerCompanyId: this.buyCompanyId,
      ...
      this.$http(url.buyerExportRIgnitionData, obj).then(res => {
    ...
    initSysData() {
      ...
      let otype = this.$store.state.oilType;
